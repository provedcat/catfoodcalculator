  ...
  try {
    ...
    if (action === "get_day") {
      ...
      return json(origin, {
        ...
        feeds: feeds.data,
        meals: meals.data,
        previousFeeds: previous.data,
      });
    }

    if (action === "update_settings") {
      const s = input.settings || {};
      if (!validDate(s.goal_start_date) || !validDate(s.goal_end_date) || s.goal_end_date < s.goal_start_date) {
        throw new Error("INVALID_GOAL_DATES");
      }
      ...
      if (!validDate(input.date)) throw new Error("INVALID_DATE");
      query = db.from("eundong_daily_records").upsert({
        recorded_date: input.date,
        weight_kg: numeric(input.weight_kg, .1, 20, true),
      }, { onConflict: "recorded_date" }).select().single();
      ...
      if (!validDate(input.date)) throw new Error("INVALID_DATE");
      const feeds = Array.isArray(input.feeds) ? input.feeds.slice(0, 3) : [];
      const rows = feeds.map((f: any) => {
        if (!validUuid(f.feed_id)) throw new Error("INVALID_FEED_ID");
        return {
          recorded_date: input.date,
          feed_slot: integer(f.feed_slot, 1, 3),
          feed_id: f.feed_id,
      ...
      const feedSlot = input.feed_slot == null ? null : integer(input.feed_slot, 1, 3);
      let snapshot: Record<string, unknown>;
      if (feedSlot) {
        const { data: f, error } = await db.from("eundong_daily_feeds")
          .select("*")
          .eq("recorded_date", input.date)
          .eq("feed_slot", feedSlot)
          .single();
